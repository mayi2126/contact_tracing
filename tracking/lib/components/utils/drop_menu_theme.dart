...
class DropMenuTheme extends StatelessWidget {
  const DropMenuTheme({super.key, required this.onSelected});
  final Function(String?) onSelected;
  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          ThemeBloc(ThemesRepository())..add(ThemesEvent()),
      child: BlocBuilder<ThemeBloc, DataState>(
        builder: (context, state) {
          ...
          return DropdownMenu<String>(
            ...
            initialSelection: themes.isNotEmpty ? themes.first['id'].toString() : null,
            width: getWidth(333),
            ...
            // Appel du callback avec l'id sélectionné
              dropdownMenuEntries: themes
                ...
                return DropdownMenuEntry<String>(
                  ...
                  label: quartier['libtheme'].isNotEmpty ? quartier['libtheme'] : ' Nom inconnu', // Affichez le name comme label
                );
              }).toList(),
          );
          } else {
